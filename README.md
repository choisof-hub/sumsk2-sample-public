...
本リポジトリは、**物流現場の『歩き回る時間』を半減させるトータルピッキング支援ツール、SUMSK2**の公開用サンプルです。
社内システムからExcelファイルとして出力される出荷伝票データを読み込み、納品先別・型式別に出荷数量を集計することで、**時間短縮と誤出荷防止**を支援します。

Python、、Tkinter、SQLiteを使用して、伝票データの取り込みから集計、Excel出力までを行います。
...
出荷時には、社内システムから出荷指示書兼納品書を発行します。

本READMEでは、社内で使用している呼称に合わせ、出荷指示書兼納品書を「伝票」と表記します。

従来の運用では、**同一納品先・同一商品であっても、複数枚の伝票に分かれて出力される場合があります。**

例えば、同じ納品先に同じ型式の商品が複数枚の伝票に分かれていると、伝票単位でピッキングすると、同じ商品の棚へ何度も足を運ばねばなりません。
現在の社内システムでは、同じ型式であっても伝票として出力される順番は入力順となるので、商品単位に出力される訳ではありません。

更に、似たような商品について大量の伝票が出力されるため、作業時に、
...
また、複数人で出荷作業を行う場合、実際に伝票を一枚ずつ手に持っていたので、
...
そこで、社内システムからEXCEL出力した伝票データを本ツールで読み込み、**納品先別・型式別に集計した一覧表を作成する仕組み**を構築しました。
...
このリポジトリで示したいのは、Pythonのプログラムを書けるという事だけではありません。

また実際の業務では、既存のシステムをすべて有償でカスタマイズする必要があるとは限りません。
...
![SUMSK2 届け先選択画面（公開用サンプル）](images/SUMSK2_納品先選択_サンプル.png)
...
![SUMSK2 合計出荷情報（標準）（公開用サンプル）](images/SUMSK2_商品選択_サンプル.png)
...
* Excelファイルから納品先名を含む出荷明細データを読み込み
* SQLiteへ取り込み
* 集計用の一時テーブルとして利用

### 2. 売上データ取込

* Excelファイルから伝票発行日時を含む売上データを読み込み
* 必要なデータをSQLiteへ取り込み
* 明細との突合に使用
